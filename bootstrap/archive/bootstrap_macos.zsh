...
typeset -r BREW_FILE="$DOTFILES_DIR/bootstrap/Brewfile"
typeset -r DOTBOT_INSTALL="$DOTFILES_DIR/install"
# For login shells, your real zprofile is in shell/zsh/zprofile and linked by Dotbot.
# Here we only ensure Homebrew shellenv in current session; we don't write to repo files.
typeset -r ZSH_PROFILE="$HOME/.zprofile"
...
install_homebrew() {
	# Ensure the ZSH_PROFILE exists
		# Runtime setup only; persistent brew shellenv is handled by your linked zprofile
	if ! command -v brew > /dev/null; then
		...
		# Configure Homebrew environment for current shell only
